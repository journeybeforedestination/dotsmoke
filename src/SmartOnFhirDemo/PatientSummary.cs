...
{
    public static PatientSummary From(Patient patient) => new(
        Name: Format(Pick<HumanName>(patient, "Patient.name.where(use='official').first()", "Patient.name.first()")),
        ...
        Address: Format(Pick<FhirAddress>(patient, "Patient.address.where(use='home').first()", "Patient.address.first()")),
    ...
    private static T? Pick<T>(Base root, params string[] expressions) where T : Base =>
        expressions.Select(e => root.Select(e).OfType<T>().FirstOrDefault()).FirstOrDefault(v => v is not null);
    ...
    private static string? Format(HumanName? name) =>
        ...
        : NullIfBlank(name.Text)
          ?? Join(" ", (name.Given ?? []).Append(name.Family));
    ...
    private static string? FormatBirthDate(Date? birthDate)
    {
        ...
        if (today < born.AddYears(age)) age--;
